...
> 스프링 시큐리티는 유저의 로그인 요청에서 파라미터를 갈채고 파라미터의 데이터를 통해 시큐리티는 로그인을 진행시킴, 로그인이 완료되면 시큐리티 세션에 유저 정보를 등록시킴, 유저정보를 원할때 사용, 스프링시큐리티에서 해쉬암호가 진행되어야 로그인 가능

> 해쉬 암호화는 고정길이의 문자열로 변환해서 진행되는 것을 의미 -> 내용이 변경되었을 경우 해쉬값이 변경되기 때문에 데이터가 위조되었는지 확인 가능
...
- 데이터 이동에 따른 공격, 하이퍼링크 방법
  + post방식으로 방어

- csrf
  + 토큰을 통해 방어
  + 스프링 시큐리티는 알아서 토큰 방어 진행

1) 카카로 로그인 (OAuth2.0 라이브러리르 쓰지 않고 구현)
- 토큰 개념
- OAuth가 무엇인지
...
- 통신을 통해 넘오는 모든 데이터를 간단하게 유효성 체크할 수 있는 방법

7) 로그 남기기 (AOP + Logger) 혹은 Sentry (클라우드 사용)

8) Goole, FaceBook 로그인 + 내 홈페이지 User 모델과 연동 (세션)

9) JWT

10) JWT와 OAuth2.0 동시 사용 (API 서버 만들기)

- 스프링부트 강좌 59강 // 강좌 목차는 62

> 톰켓이 시작되면 필터를 거친다. 필터에서는 인코딩, 기본적으로 걸러야 할 내용들을 거른다. 다음에 디스패처가 적용된다. 디스패처는 사용자가 요청한 주소를 적절한 컨트롤러와 연결한다. 다음으로 컨트롤러, 서비스, JPA, 영속성 컨텍스트 등이 연결된다. 컨트롤러, 서비스, JPA, 영속성 컨텍스트는 사용자의 요청에 따라 연결된다. 각각의 스레드를 통해 보여진다. datasource는 톰캣이 시작되면 자동적으로 연결된다. 또한 database는 datasource와 연결되어 있다. view Resolver, 세션, 인터셉터는 톰캣이 시작되면 연결된다.
